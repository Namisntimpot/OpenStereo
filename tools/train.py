...
def setup_ddp(backend='nccl'):
    ...
    cuda_id = rank % num_gpus
    torch.cuda.set_device(local_rank)
    device = torch.device("cuda", local_rank)
    dist.init_process_group(backend=backend, device_id=device, rank=rank, world_size=world_size)  # device_id must be explicitly specified to avoid warnings
    # dist.init_process_group(backend=backend, rank=rank, world_size=world_size)
    print("RANK", rank, "LOCAL_RANK", local_rank, "WORLD_SIZE", world_size,
      "CUDA_VISIBLE_DEVICES", os.environ.get("CUDA_VISIBLE_DEVICES"))
    return local_rank, rank, group_rank, world_size


def main():
    args, cfgs = parse_config()
    if args.dist_mode:
        local_rank, global_rank, group_rank, world_size = setup_ddp()
    else:
        local_rank = 0
        global_rank = 0
        group_rank = 0
    ...
    if os.path.exists(args.output_dir) and args.extra_tag != 'debug' and cfgs.MODEL.CKPT == -1:
        ...
        logger.warning('There is already an exp with this name, may overwrite previous results!')
